...
from model import Weights_Normal, VGG19_PercepLoss
from model import GeneratorFunieGAN, DiscriminatorFunieGAN
from utils import GetTrainingPairs, GetValImage
...
dataloader = DataLoader(
    GetTrainingPairs(dataset_pathm dataset_name, transforms_= transforms_),
...
val_dataloader = DataLoader(
    GetValImage(dataset_path, dataset_name, transforms_ = transforms_, sub_dirs='validation'),
    batch_size = 4,
    shuffle = True,
    num_workers = 1,
)
...
for epoch in range(epoch, num_epochs):
    for i, batch in enumerate(dataloader):
        imgs_distord = Variable(batch['A'].type(Tensor))
        img_good_gt = Variable(batch['B'].type(Tensor))

        valid = Variable(Tensor(np.ones((imgs_distord.size(0),*patch))), requires_grad = False)
        fake = Variable(Tensor(np.zeros((imgs_distord.size(0),*patch))), requires_grad=False)

        optimizer_D.zero_grad()
        imgs_fake = generator(imgs_distord)
        pred_real = discriminator(img_good_gt, imgs_distord)

        loss_real = Adv_cGAN(pred_real, valid)
        pred_fake = discriminator(imgs_good_gt, imgs_distord)
        ...
        imgs_fake = generator(imgs_distord)
        pred_fake = discriminator(imgs_fake, imgs_distord)
        loss_GAN = Adv_cGAN(pred_fake, valid)
        loss_1 = L1_G(imgs_fake, img_good_gt)
        loss_con = L_vgg(imgs_fake, img_good_gt)
